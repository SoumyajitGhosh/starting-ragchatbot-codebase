...
def test_execute_formats_results_with_course_and_lesson_headers(
    search_tool, mock_vector_store, sample_search_results
):
...
def test_execute_tracks_sources_with_links(
    ...
    mock_vector_store.get_lesson_link.assert_any_call(
        "Building Towards Computer Use with Anthropic", 0
    )
    mock_vector_store.get_lesson_link.assert_any_call(
        "Building Towards Computer Use with Anthropic", 1
    )
...
def test_execute_no_results_message_includes_filters(search_tool, mock_vector_store):
    ...
    result = search_tool.execute(
        query="nonexistent topic", course_name="MCP", lesson_number=3
    )
...
def test_execute_passes_query_course_lesson_to_store(
    search_tool, mock_vector_store, sample_search_results
):
    mock_vector_store.search.return_value = sample_search_results
...
def test_execute_lesson_without_link_does_not_crash(search_tool, mock_vector_store):
    ...
    assert search_tool.last_sources == [
        {"text": "Some Course - Lesson 5", "link": None}
    ]


def test_execute_result_without_lesson_number_uses_course_link(
    ...
    assert search_tool.last_sources == [
        {"text": "Some Course", "link": "https://example.com/course"}
    ]
